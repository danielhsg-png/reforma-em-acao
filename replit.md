...
## Pendências Futuras (não urgentes)
- **Webhook Pagar.me — auth falhando para `subscription.canceled`**: histórico de logs mostra 100% de falha de autenticação (401) nos webhooks automáticos de `subscription.canceled` vindos de IPs do Pagar.me, enquanto `charge.paid`/`order.paid` autenticam corretamente na maioria das vezes. A lógica de Basic Auth no servidor (`checkPagarmeBasicAuth` em `server/routes.ts`) está correta — o problema é externo, provavelmente uma configuração de webhook separada no painel do Pagar.me para eventos de assinatura sem o cabeçalho de Basic Auth configurado. Ação recomendada: no painel do Pagar.me, conferir se há múltiplas configurações de webhook e garantir que todas usem as credenciais de `PAGARME_WEBHOOK_USER`/`PAGARME_WEBHOOK_PASSWORD`. Não afeta o cancelamento feito pelo botão no app (fluxo próprio, funcionando e testado).
